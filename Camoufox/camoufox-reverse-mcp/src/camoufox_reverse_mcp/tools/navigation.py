...
from ..utils.worlds import wrapped_main_world_init_script
...
async def navigate(
    ...
    try:
        ...
        if clear_network_capture:
            try:
                cleared_count = browser_manager.clear_network_capture()
                if cleared_count > 0:
...
async def _inject_hook_by_name(name: str) -> tuple[bool, str]:
    ...
    try:
        # Camoufox runs ordinary init scripts in an isolated world; preset hook
        # files patch the page's real window, so wrap them for the main world.
        js = wrapped_main_world_init_script(js)
    except Exception as e:
        return False, f"main-world wrap failed: {e}"
    try:
...
async def _reset_browser_state_impl(
    ...
    try:
        ...
        if clear_active_routes:
            try:
                from .instrumentation import _active_routes, _source_site_registry, _stop
