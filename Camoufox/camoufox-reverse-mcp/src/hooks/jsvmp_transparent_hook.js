...
(function () {
    ...
    function inTargetScript(stack) {
        if (!CFG.scriptUrl) {
            // No scriptUrl filter — accept all calls including console/eval testing.
            return true;
        }
        if (!stack) return false;
        if (stack.indexOf(CFG.scriptUrl) !== -1) return true;
        // When scriptUrl IS set, only record calls whose stack references that
        // script. eval/debugger bypass is intentionally disabled here because it
        // causes false-positive log inflation from unrelated eval contexts on the
        // same page.  For console testing, call with script_url='' instead.
        return false;
    }
