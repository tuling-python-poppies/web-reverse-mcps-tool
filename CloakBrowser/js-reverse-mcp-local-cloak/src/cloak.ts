...
import {resolveConfiguredCloakBinaryPath} from './cloakBinaryManager.js';
...
function resolveExplicitBinaryPath(binaryPath: string): string {
  ...
  if (!existsSync(resolved)) {
    throw new Error(
      `CloakBrowser binary does not exist: ${resolved}. ` +
        'Run update_browser_binary({dry_run:true}) to inspect the default parent-directory install target.',
    );
  }
  return resolved;
}
...
export async function setupCloak(
  ...
  const executablePath = resolveExplicitBinaryPath(
    await resolveConfiguredCloakBinaryPath(binaryPath),
  );
