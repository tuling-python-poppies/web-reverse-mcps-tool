...
> {
  ...
  /**
   * Binary/configuration tools can run without opening a browser context.
   * Defaults to true for all browser-facing tools.
   */
  requiresBrowserContext?: boolean;
  /** Override the default MCP tool timeout for long-running operations. */
  timeoutMs?: number;
  schema: Schema;
  handler: (
