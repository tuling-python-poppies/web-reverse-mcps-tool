...
import {
  ...
  checkBrowserUpdate as checkCloakBrowserUpdate,
  getInstalledBinaryInfo,
  resolveConfiguredCloakBinaryPath,
  updateBrowserBinary as updateCloakBrowserBinary,
} from '../cloakBinaryManager.js';
...
// CLI launch defaults (set via setLaunchDefaults from main.ts)
let _cliHeadless = false;
let _cliCloak = false;
 ...
 */
export function setCloakConfig(
...
/** Called by main.ts so launch_browser can resolve effective headless/cloak. */
export function setLaunchDefaults(defaults: {
  headless?: boolean;
  cloak?: boolean;
}): void {
  if (defaults.headless !== undefined) _cliHeadless = !!defaults.headless;
  if (defaults.cloak !== undefined) _cliCloak = !!defaults.cloak;
}

function resolveEffectiveLaunchState(): {
  cloak: boolean;
  headless: boolean;
  cloakBinaryPath?: string;
} {
  const overrides = getRuntimeLaunchOverrides();
  const last = getLastLaunchSnapshot();
  const cloak = overrides?.cloak ?? last?.cloak ?? _cloakActive ?? _cliCloak;
  const headless = overrides?.headless ?? last?.headless ?? _cliHeadless;
  const cloakBinaryPath = cloak
    ? (overrides?.cloakBinaryPath ?? last?.cloakBinaryPath ?? _cloakBinaryPath)
    : undefined;
  return {cloak: !!cloak, headless: !!headless, cloakBinaryPath};
}

export interface LaunchBrowserPreflightResult {
  status: 'already_running' | 'relaunched';
  target: string;
  path: string | null;
...
let _launchBrowserPreflight: LaunchBrowserPreflightResult | undefined;

/**
 * Must run BEFORE getContext() for launch_browser.
 * Applies headless/cloak overrides and closes the current browser so the
 * subsequent getContext() relaunches in-process with the new options
 * (same tool call, no visible headful flash from CLI defaults).
 */
export async function preflightLaunchBrowser(params: {
  headless?: boolean;
  cloakBinaryPath?: string;
  humanize?: boolean;
}): Promise<LaunchBrowserPreflightResult> {
  if (params.humanize !== undefined) {
    _humanizeEnabled = params.humanize;
  }

  const requestedHeadless = params.headless;
  const requestedCloakPath = params.cloakBinaryPath;
  const engineChangeRequested = requestedCloakPath !== undefined;
  const headlessChangeRequested = requestedHeadless !== undefined;
  const current = resolveEffectiveLaunchState();

  if (!engineChangeRequested && !headlessChangeRequested) {
    _launchBrowserPreflight = {
      status: 'already_running',
      target: current.cloak ? 'CloakBrowser' : 'normal Chrome',
      path: current.cloakBinaryPath ?? null,
      effective_headless: current.headless,
      cloak_active: current.cloak,
      note: 'Browser already running. Pass headless and/or cloakBinaryPath to relaunch with new options.',
    };
    return _launchBrowserPreflight;
  }

  let nextCloak = current.cloak;
  let nextCloakPath = current.cloakBinaryPath;
  let nextHeadless = current.headless;

  if (engineChangeRequested) {
    if (requestedCloakPath === '') {
      nextCloak = false;
      nextCloakPath = undefined;
    } else {
      nextCloak = true;
      nextCloakPath = requestedCloakPath;
    }
  }
  if (headlessChangeRequested) {
    nextHeadless = !!requestedHeadless;
  }

  const engineChanged =
    nextCloak !== current.cloak ||
    (nextCloak && nextCloakPath !== current.cloakBinaryPath);
  const headlessChanged = nextHeadless !== current.headless;

  if (!engineChanged && !headlessChanged) {
    _launchBrowserPreflight = {
      status: 'already_running',
      target: current.cloak ? 'CloakBrowser' : 'normal Chrome',
      path: current.cloakBinaryPath ?? null,
      effective_headless: current.headless,
      cloak_active: current.cloak,
      note: 'Requested headless/cloak options already match the effective launch state; no relaunch.',
    };
    return _launchBrowserPreflight;
  }
  ...
  if (engineChangeRequested) {
    if (requestedCloakPath === '') {
      ...
      patchRuntimeLaunchOverrides({
        cloak: true,
        cloakBinaryPath: requestedCloakPath,
      });
    }
  }
  if (headlessChangeRequested) {
    patchRuntimeLaunchOverrides({headless: nextHeadless});
  }

  const target = nextCloak ? 'CloakBrowser' : 'normal Chrome';
  await closeBrowser(
    `manual close via launch_browser (${target}, headless=${nextHeadless})`,
  );

  _launchBrowserPreflight = {
    status: 'relaunched',
    target,
    path: nextCloak ? (nextCloakPath ?? null) : null,
    effective_headless: nextHeadless,
    cloak_active: nextCloak,
    note: 'Browser relaunched in this tool call with the new headless/cloak options.',
  };
  return _launchBrowserPreflight;
}

export function takeLaunchBrowserPreflight():
  | LaunchBrowserPreflightResult
  | undefined {
  const result = _launchBrowserPreflight;
  _launchBrowserPreflight = undefined;
  return result;
}
...
export const browserBinaryInfo = defineTool({
  ...
  handler: async (_request, response) => {
    // Priority: runtime override > last launch > CLI arg > portable config > MCP parent.
    const effective = resolveEffectiveLaunchState();
    const cloakBinaryPath = await resolveConfiguredCloakBinaryPath(
      effective.cloakBinaryPath,
    );
    const binaryInfo = await getInstalledBinaryInfo({
      installDir: path.dirname(cloakBinaryPath),
      binaryPath: cloakBinaryPath,
    });

    appendJson(response, {
      mode: 'patchright',
      cloak_active: effective.cloak,
      cloak_binary_path: cloakBinaryPath,
      cloak_binary_installed: binaryInfo.installed,
      cloak_binary_version: binaryInfo.version,
      cloak_install_dir: binaryInfo.installDir,
      effective_headless: effective.headless,
      cli_headless: _cliHeadless,
      config_file: BROWSER_CONNECTION_CONFIG_FILE,
      protected_paths: binaryInfo.protectedPaths,
      note:
        'cloak_active: true = CloakBrowser is currently running. ' +
        'effective_headless: resolved headless after runtime overrides (CLI default if unset). ' +
        'cloak_binary_path: resolved CloakBrowser executable path (CLI/runtime > portable config > MCP parent). ' +
        'config_file: portable config supports {"cloakInstallDir":"{mcpParent}","cloakBinaryName":"chrome.exe"}. ' +
        'Switch at runtime via launch_browser({headless, cloakBinaryPath}).',
    });
  },
});
...
export const checkBrowserUpdate = defineTool({
  ...
  description:
    'Check whether the parent-directory CloakBrowser binary can be updated.',
  annotations: {category: ToolCategory.DEBUGGING, readOnlyHint: true},
  requiresBrowserContext: false,
  timeoutMs: 45_000,
  schema: {
    ...
    install_dir: zod
      ...
      .optional()
      .describe(
        'Optional CloakBrowser install directory. Relative paths and {mcpParent}/{mcpRoot} tokens are supported; default is {mcpParent}.',
      ),
  },
  handler: async (request, response) => {
    const result = await checkCloakBrowserUpdate({
      version: request.params.version,
      installDir: request.params.install_dir,
    });
    appendJson(response, {
      supported: result.supported,
      update_available: result.updateAvailable,
      current_version: result.version,
      binary_product_version: result.binaryProductVersion,
      marker_version: result.markerVersion,
      latest_version: result.latestVersion,
      install_dir: result.installDir,
      binary_path: result.binaryPath,
      installed: result.installed,
      release: result.release,
      protected_paths: result.protectedPaths,
...
export const updateBrowserBinary = defineTool({
  ...
  description:
    'Safely update the parent-directory CloakBrowser binary without deleting this MCP directory.',
  annotations: {category: ToolCategory.DEBUGGING, readOnlyHint: false},
  requiresBrowserContext: false,
  timeoutMs: 20 * 60_000,
  schema: {
    ...
    dry_run: boolParam().optional().default(false),
    skip_checksum: boolParam().optional().default(false),
    install_dir: zod
      .string()
      .nullable()
      .optional()
  ...
  handler: async (request, response) => {
    if (isBrowserRunning() && !request.params.close_running_browser) {
      const result = await checkCloakBrowserUpdate({
        version: request.params.version,
        installDir: request.params.install_dir,
      });
      appendJson(response, {
        status: 'blocked',
        reason:
          'The MCP browser is currently running. Re-run with close_running_browser:true before updating chrome.exe.',
        update_available: result.updateAvailable,
        current_version: result.version,
        latest_version: result.latestVersion,
        install_dir: result.installDir,
    ...
    if (request.params.close_running_browser) {
      await closeBrowser('manual close via update_browser_binary tool');
    }

    const result = await updateCloakBrowserBinary({
      version: request.params.version,
      force: request.params.force,
      dryRun: request.params.dry_run,
      skipChecksum: request.params.skip_checksum,
      installDir: request.params.install_dir,
    });
    appendJson(response, {
      ...
      protected_paths: result.protectedPaths,
      release: result.release,
      planned_download_url: result.plannedDownloadUrl,
      staging_dir: result.stagingDir,
      backup_dir: result.backupDir,
      downloaded_archive: result.downloadedArchive,
      removed_downloaded_archive: result.removedDownloadedArchive,
      archive_cleanup_error: result.archiveCleanupError,
      copied_files: result.copiedFiles,
      backed_up_files: result.backedUpFiles,
      config_file: result.configFile,
      note:
        result.status === 'dry_run'
          ? 'Dry run did not download or write files. Re-run with dry_run:false to update.'
          : undefined,
    });
  },
});
...
export const launchBrowser = defineTool({
  ...
  description:
    ...
    'headless:true|false closes the current browser and relaunches with that mode in the same tool call. ' +
    'cloakBinaryPath switches to CloakBrowser; empty string switches back to normal Chrome. ' +
    'headless and cloakBinaryPath may be combined. ' +
    'Omit both to report current state without relaunching.',
  annotations: {category: ToolCategory.NAVIGATION, readOnlyHint: false},
  schema: {
    ...
    cloakBinaryPath: zod
      ...
      .describe(
        'Path to CloakBrowser executable. When provided, closes current browser and relaunches with CloakBrowser. ' +
          ...
          'Omit to keep the current engine (may still relaunch if headless changes).',
      ),
  },
  handler: async (request, response, context) => {
    // Preflight (main.ts) already applied overrides + closed + relaunched via
    // getContext(). Report that result here with the live page list.
    const preflight =
      takeLaunchBrowserPreflight() ??
      (await preflightLaunchBrowser({
        headless: request.params.headless,
        cloakBinaryPath: request.params.cloakBinaryPath,
        humanize: request.params.humanize,
      }));
    const last = getLastLaunchSnapshot();
    const effective = resolveEffectiveLaunchState();
    const payload: Record<string, unknown> = {
      ...
      path: preflight.path,
      effective_headless: last?.headless ?? preflight.effective_headless,
      cloak_active: last?.cloak ?? preflight.cloak_active,
      note: preflight.note,
      pages: context
        .getPages()
    ...
    if (preflight.status === 'already_running') {
      payload.cli_flag_mapping = {
        headless: '--headless (CLI default; runtime headless also supported)',
        locale: '--locale zh-CN',
      ...
      payload.requested_matches_effective =
        (request.params.headless === undefined ||
          !!request.params.headless === effective.headless) &&
        (request.params.cloakBinaryPath === undefined ||
          (request.params.cloakBinaryPath === ''
            ? !effective.cloak
            : effective.cloak));
    }
    appendJson(response, payload);
  },
});
...
export const closeBrowserTool = defineTool({
  name: 'close_browser',
  description:
    'Close the managed browser and release all resources, clearing runtime cloak/headless overrides. ' +
    'The next tool call will automatically relaunch with CLI defaults (normal Chrome unless --cloak was set).',
  annotations: {category: ToolCategory.NAVIGATION, readOnlyHint: false},
  schema: {},
  handler: async (_request, response) => {
    ...
    appendJson(response, {
      status: 'closed',
      note: 'Browser closed and runtime overrides cleared. Next tool call will launch with CLI defaults.',
      effective_headless: _cliHeadless,
      cloak_active: _cliCloak,
    });
  },
});
