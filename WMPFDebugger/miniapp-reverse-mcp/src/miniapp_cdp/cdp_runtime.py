...
class CDPRuntime:
    ...
    def select_default_app_target(self, targets: list[TargetState]) -> TargetState:
        # Priority 1: AppService target (logic layer - where XHR/Fetch requests originate)
        appservice_targets = [
            t
            for t in targets
            if t.target_type == "page"
            and "servicewechat.com" in t.url
            and "appservice.html" in t.url
            and "preload-" not in t.url
        ]
        if appservice_targets:
            return appservice_targets[0]

        # Priority 2: AppService with preload (less ideal but still logic layer)
        appservice_preload = [
            t
            for t in targets
            if t.target_type == "page"
        ...
        if appservice_preload:
            return appservice_preload[0]

        # Priority 3: WebView non-preload with AppIndex title
        preferred_exact = [
            t
            for t in targets
            if t.target_type == "page"
        ...
        # Priority 4: WebView non-preload
        ...
        # Priority 5: Any servicewechat page
        servicewechat_pages = [
            t
            for t in targets
            if t.target_type == "page"
