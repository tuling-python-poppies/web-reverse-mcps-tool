...
class MiniappCDPMCP:
    ...
    async def get_request_post_data(self, request_id: str, max_size: int = 20000) -> str:
        return await get_request_post_data(
            self.network_capture,
            request_id=request_id,
            max_size=max_size,
        )
    ...
    async def set_event_listener_breakpoint(self, event_name: str, target_name: str | None = None) -> str:
        return await set_event_listener_breakpoint(self.network_capture, event_name, target_name)

    async def remove_event_listener_breakpoint(self, event_name: str, target_name: str | None = None) -> str:
        return await remove_event_listener_breakpoint(self.network_capture, event_name, target_name)

    async def step(self, action: str) -> str:
        return await step(self.network_capture, action)

    async def get_runtime_events(self, event_type: str = "console", clear: bool = False, limit: int = 50) -> str:
        return await get_runtime_events(self.network_capture, event_type=event_type, clear=clear, limit=limit)

    async def start_cpu_profile(self, sampling_interval: int | None = None) -> str:
        return await start_cpu_profile(self.network_capture, sampling_interval=sampling_interval)

    async def stop_cpu_profile(self, limit: int = 30, include_profile: bool = False) -> str:
        return await stop_cpu_profile(self.network_capture, limit=limit, include_profile=include_profile)

    async def precise_coverage(self, action: str = "start", reset: bool = False, limit: int = 100) -> str:
        return await precise_coverage(self.network_capture, action=action, reset=reset, limit=limit)
