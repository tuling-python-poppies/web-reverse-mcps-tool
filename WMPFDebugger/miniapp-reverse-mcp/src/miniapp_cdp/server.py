...
@mcp.tool(
    ...
    description="Get request POST data via CDP Network.getRequestPostData for a captured request.",
)
async def get_request_post_data(request_id: str, max_size: int = 20000) -> str:
    return await app.get_request_post_data(request_id=request_id, max_size=max_size)
...
@mcp.tool(
    ...
    description="Set a CDP DOM event listener breakpoint, e.g. click, submit, input, keydown.",
)
async def set_event_listener_breakpoint(event_name: str, target_name: str | None = None) -> str:
    return await app.set_event_listener_breakpoint(event_name=event_name, target_name=target_name)


@mcp.tool(
    name="remove_event_listener_breakpoint",
    description="Remove a CDP DOM event listener breakpoint set by set_event_listener_breakpoint.",
)
async def remove_event_listener_breakpoint(event_name: str, target_name: str | None = None) -> str:
    return await app.remove_event_listener_breakpoint(event_name=event_name, target_name=target_name)
...
@mcp.tool(
    ...
    description="Get captured Runtime console, exception, or execution context events.",
)
async def get_runtime_events(event_type: str = "console", clear: bool = False, limit: int = 50) -> str:
    return await app.get_runtime_events(event_type=event_type, clear=clear, limit=limit)


@mcp.tool(
    name="start_cpu_profile",
    description="Start CDP CPU profiling for locating hot AppService/WebView functions.",
)
async def start_cpu_profile(sampling_interval: int | None = None) -> str:
    return await app.start_cpu_profile(sampling_interval=sampling_interval)
...
async def stop_cpu_profile(limit: int = 30, include_profile: bool = False) -> str:
    return await app.stop_cpu_profile(limit=limit, include_profile=include_profile)


@mcp.tool(
    name="precise_coverage",
    description="Control CDP precise coverage: action=start/take/stop.",
)
async def precise_coverage(action: str = "start", reset: bool = False, limit: int = 100) -> str:
    return await app.precise_coverage(action=action, reset=reset, limit=limit)
