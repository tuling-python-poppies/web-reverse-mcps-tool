...
def format_requests_markdown(data: dict) -> str:
    ...
    if data.get("mode") == "detail":
        ...
        if res_headers:
            ...
            for k, v in res_headers.items():
                md.append(f"- {k}: {v}")
        extra_req = req.get("request_extra_info")
        if extra_req:
            import json
            md.append("### Request Extra Info")
            md.append(json.dumps(extra_req, ensure_ascii=False, indent=2)[:5000])
        extra_res = req.get("response_extra_info")
        if extra_res:
            import json
            md.append("### Response Extra Info")
            md.append(json.dumps(extra_res, ensure_ascii=False, indent=2)[:5000])
        failure = req.get("failure")
        if failure:
            import json
            md.append("### Failure")
            md.append(json.dumps(failure, ensure_ascii=False, indent=2))
    ...
    if not reqs:
        ...
        for r in reqs:
            ...
            status = f"[success - {status_code}]" if status_code else "[failed]" if r.get('loading_failed') else "[pending]"
...
async def get_request_post_data(
    ...
    try:
        result = await asyncio.wait_for(
            network_capture.get_request_post_data(request_id=request_id),
            timeout=5.0,
        )
    except asyncio.TimeoutError:
        return "<timed out fetching post data>"
    except Exception as e:
        return f"<not available: {e}>"
    post_data = result.get("postData") or ""
    if not post_data:
        return "<empty post data>"
    if max_size >= 0 and len(post_data) > max_size:
        return post_data[:max_size] + "\n... <truncated>"
    return post_data
